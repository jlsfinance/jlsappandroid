...
export class PdfGenerator {
    ...
    static async generateLoanAgreement(record: Record, customer: Customer, company: Company) {
        const pdf = new jsPDF();
        ...
        pdf.text(`Interest Rate: ${record.interestRate}% p.a.`, 140, 75);

        pdf.text(`Tenure: ${record.tenure} Months`, 20, 85);
        pdf.text(`EMI Amount: ${formatCurrency(record.emi)}`, 140, 85);
        ...
        await savePdf(pdf, `Loan_Agreement_${record.id}.pdf`);
    }

    static async generateReceipt(record: Record, emi: Emi, customer: Customer, company: Company) {
        const pdf = new jsPDF({
        ...
        await savePdf(pdf, `Receipt_${record.id}_EMI${emi.emiNumber}.pdf`);
    }

    static async generateLoanCard(record: Record, customer: Customer, company: Company) {
        const pdf = new jsPDF();
        ...
        pdf.text(`Interest: ${record.interestRate}%`, 70, y);
        pdf.text(`Tenure: ${record.tenure}M`, 110, y);
        pdf.text(`EMI: ${formatCurrency(record.emi)}`, 150, y);
        ...
        await savePdf(pdf, `LoanCard_${record.id}.pdf`);
    }
        ...
        records.forEach(record => {
            ...
            record.repaymentSchedule?.filter(e => e.status === 'Paid').forEach(e => {
                transactions.push({
                    ...
                    type: 'EMI PAYMENT',
                    ref: `Record #${record.id} - EMI #${e.emiNumber}`,
                    debit: 0,
    ...
    static async generateNoDuesCertificate(record: Record, customer: Customer, company: Company) {
        ...
        const certificateText = `This is to certify that Mr./Ms. ${customer.name} having Record Account Number ${record.id} has successfully repaid the entire record amount along with all applicable interest and charges.

As on ${format(new Date(), 'dd-MMM-yyyy')}, there are no outstanding dues against the aforementioned record account.

The company has no further claim or lien on any security/collateral provided for this record. This certificate is issued at the request of the borrower for their records.`;
